<template>
	<div>
		<div id="settlement-account-virtual-input" @click.stop="readyToInput">
			<div class="input-container" tabindex="-1" @blur.stop="clearCursorFlash($event)">
				<div class="input-area">
					<span class="cursor"></span>
				</div>
				<div class="right-space" @click="moveCursor($event)">
					<span v-if="showText" class="gray-text" v-text="placeholder"></span>
				</div>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...
		moveCursor(event) {
			...
			if (event.currentTarget.className == "right-space") {
				if (
					...
					this.inputArea.insertBefore(
						this.inputArea.lastElementChild,
						ele
					);
				...
				if (!tempEle || tempEle.nodeName == "#text") {
					...
					let ele = this.inputArea.replaceChild(
						event.currentTarget,
						cursor
					); //把光标替换成当前元素
		...
		setCursorFlash() {
			let cursor = this.$el.getElementsByClassName("cursor")[0];
			let inputContainer = this.$el.getElementsByClassName(
				"input-container"
			)[0];
		...
		getValue(value) {
			...
			if (
				(isBefore == 0 || inputStr.search(/\.\d{2}/) == -1) &&
				(value !== "." || inputStr.search(/\./) == -1)
			) {
...
[data-dpr="2"] #settlement-account-virtual-input {
	font-size: 70px;
}

[data-dpr="2.5"] #settlement-account-virtual-input {
	font-size: 88px;
}

[data-dpr="2.75"] #settlement-account-virtual-input {
	font-size: 96px;
}

[data-dpr="3"] #settlement-account-virtual-input {
	font-size: 105px;
}

[data-dpr="4"] #settlement-account-virtual-input {
	font-size: 140px;
}

#settlement-account-virtual-input .input-container {
	margin-top: 0.08rem;
	overflow: hidden;
}

#settlement-account-virtual-input .input-container .input-area {
...
#settlement-account-virtual-input .input-container .right-space {
	overflow: hidden;
	height: 0.93333333rem;
	line-height: 0.93333333rem;
}

#settlement-account-virtual-input .input-container .right-space .gray-text {
	color: #999999;
}

#settlement-account-virtual-input .input-container span {
	display: inline-block;
}

#settlement-account-virtual-input .cursor {
	width: 2px;
	height: 0.93333333rem;
...
#settlement-account-virtual-input .space {
	position: relative;
	z-index: 999;
	width: 4px;
	height: 0.93333333rem;
...
#settlement-account-virtual-input .hidden {
	visibility: hidden;
}
</style>